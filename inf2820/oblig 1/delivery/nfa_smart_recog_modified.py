...
# This is probably unnecesserly long, but exception handling in list
# comprehension gave me a headache.
def nextStates(nfa, curStates, tape, index):
    newStates = []
    for e in nfa.edges:
        stateExist = False
        onTape = False
        ...
        if (tape[index] == e[1]):
            onTape = True
        if nfa.abrs.get(e[1]) is not None:
            if tape[index] in nfa.abrs.get(e[1]):
                onAbrs = True
        if stateExist and (onTape or onAbrs):
            newStates.append(e[2])
    return newStates
...
def nrec(tape, nfa, trace=0):
    ...
    while True:
        ...
        if index == len(tape): # End of input reached
            ...
            # Call the function and receive a set
            states = nextStates(nfa, states, tape, index)
            index += 1 
